...
function drawProject(projects) {
    for (let project of projects) {
        if (project.expired) {
            ...
            block.classList.add('block', 'project');
            ...
            projectButtons.innerHTML = `<a class="block button">Отправить</a><a class="block button">Еще</a>`;
    ...
    if (projects.length === 0) {
        document.querySelector('.block.empty').classList.remove('hidden');
    }
}
